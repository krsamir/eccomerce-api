  ...
  return winston.createLogger({
    ...
    format: winston.format.printf(({ level, message }) => {
      ...
      if (
        ...
        return `[${new Date().toISOString()}] [correlationId=${correlationId}]  [FILENAME: ${paths}] ${level.toUpperCase()} [PATH=${path}] [METHOD=${method}] ${
          message === CONSTANTS.ROUTE_LOGS ? "" : `: ${message}`
        }`;
      }
      return `[${new Date().toISOString()}] ${correlationId !== "N/A" ? `[correlationId=${correlationId}]` : ""} [FILENAME: ${paths}] ${level.toUpperCase()}: ${message}`;
    }),
